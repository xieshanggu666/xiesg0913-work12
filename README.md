...
- **工艺实验**：以一条已有轨迹为基准复制实验方案，只允许修改指定帧区间内的**一项**旋钮参数，
  ...
  结果较多时列表按每段 8 条轻量分段（上一段 / 下一段 + 段号与本段条目范围）；切换任意筛选或排序
  自动回到首段，删除记录 / 保存结论后停在仍有效的当前段（末段删空自动回退）并刷新数量。
...
7. 右栏「工艺实验」：以当前轨迹为基准，设置帧区间与一项旋钮实验值后点「分别运行」，
   ...
   并切换时间正序 / 倒序；命中数与总数实时显示，「清空筛选条件」一键还原。记录较多时每段只加载
   8 条，用「上一段 / 下一段」翻段；切换筛选或排序自动回到首段，删除记录 / 保存结论后保持在当前段
   （当前段删空自动回退到最后一个有效段），数量与段范围同步刷新。
